...
│  ┌─────────────────────────────────────────────┐            │
   ...
   │  │  (Pure observer - no RAL or ref model)   │            │
   │  └──────────────────────────────────────────┘            │
└─────────────────────────────────────────────────────────────┘
```
...
│  ┌─────────────────────────────────────────────┐            │
   ...
   │  │  • m_ap_mode  : uvm_analysis_port (mode) │            │
   │  └──────────────────────────────────────────┘            │
└─────────────────────────────────────────────────────────────┘
```
...
└──────────────────────────────────────────────────────────────────────────────┘
  ...
  │      m_ap     ●──┼────────────────────────────────────────┼──► cp_addr_op    │
  │                  │                                        │                  │
  │   m_ap_mode   ●──┼──► CpmScoreboardModeImp ──► Scoreboard.write_mode()       │
  │                  │    (broadcasts mode on MODE register writes)              │
  └──────────────────┘                                        └──────────────────┘
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  │         │                                                               │
  │  7. do_readback()     ─► RAL read counters, log values                  │
  │         │                  (invariant check moved to test)              │
  │         │                                                               │
  │  wait_for_idle()      ─► uvm_event_pool EVT_SCOREBOARD_IDLE             │
  │                          (event-based sync, no fixed delays)            │
  └─────────────────────────────────────────────────────────────────────────┘
```
...
2. **Mode Tracking via TLM Analysis Ports**: Register monitor broadcasts mode changes via `m_ap_mode` to `CpmScoreboardModeImp`, which routes them to the scoreboard. The packet monitor is a pure observer with no RAL access.

3. **Event-Based Synchronization**: Uses `uvm_event_pool` with `EVT_SCOREBOARD_IDLE` and `EVT_SOFT_RESET` events instead of hard-coded delays. Scoreboard triggers idle event when expected queue empties; virtual sequence and test wait for this event with timeout protection.

4. **Reference Model Reads from RAL**: The reference model (`CpmRefModel`) reads mask, add_const, drop_en, and drop_opcode from the RAL model's mirrored values to ensure accurate predictions.

5. **ID+OPCODE Packet Matching**: Scoreboard matches packets by ID+OPCODE combination to handle reordering caused by different mode latencies (PASS=0, XOR=1, ADD=2, ROT=1 cycles).

6. **Coverage Traffic Sequence**: `CpmCoverageTrafficSeq` explicitly generates all 16 opcodes to guarantee 100% cross coverage when run in each mode.

7. **Robust Reset Wait**: All drivers/monitors use clocked reset wait (`do @(posedge clk) while (rst);`) to ensure proper synchronization.

8. **SVA Drop Awareness**: The bounded liveness SVA assertion uses shadow registers to track DROP_CFG and automatically disables when packets are intentionally dropped.

---

## Complete Hierarchy & Connection Map
...
└── CpmBaseTest / CpmMainTest / CpmSmokeTest / CpmRalResetTest
    ...
    └── m_env             : CpmEnv
        ...
        │   └── m_export_mode     : CpmScoreboardModeImp (uvm_subscriber)
        │
...
| CpmRegMonitor | m_ap_mode | ──► | CpmScoreboardModeImp | analysis_export |
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  └── Runs on m_packet_seqr (CpmPacketSequencer):
      ...
      │
      ├── CpmStressSeq             (stress traffic)
      │   ├── m_num_packets = 200
      │   └── Creates CpmPacketTxn via repeat(m_num_packets)
      │
