...
class CpmRegMonitor extends uvm_monitor;
    ...
    // TLM Analysis Ports
    // ============================================================================
    uvm_analysis_port #(CpmRegTxn) m_ap;           // All register transactions
    uvm_analysis_port #(cpm_mode_e) m_ap_mode;     // Mode changes only (for packet monitor)
    ...
    // Broadcasts mode changes via m_ap_mode for packet monitor to track
    // ============================================================================
    virtual task monitor_reg_transaction();
        ...
        if (m_vif.write_en) begin
            ...
            // Broadcast mode changes to subscribers (e.g., packet monitor)
            // This allows packet monitor to track current mode without accessing RAL
            if (txn.m_addr == CPM_REG_MODE_ADDR) begin
                cpm_mode_e new_mode;
                new_mode = cpm_mode_e'(txn.m_wdata[1:0]);  // MODE is bits [1:0]
                `uvm_info("MON", $sformatf("MODE change detected: %s", new_mode.name()), UVM_MEDIUM)
                m_ap_mode.write(new_mode);
            end
        end else begin
