...
class CpmPacketMonitor extends uvm_monitor;
    ...
    // Note: mode_at_accept is NOT set here - the scoreboard tracks mode from
    // the register monitor and sets it when the packet is received.
    // This keeps the monitor "pure" - it only observes interface signals.
    // ============================================================================
    virtual task monitor_input_packet();
        ...
        // Note: txn.m_mode_at_accept will be set by scoreboard
        ...
        `uvm_info("MON", $sformatf("Input packet: %s", txn.convert2string()), UVM_HIGH)
    endtask
